...
BEGIN {
    ...
    if ( $] > 5.007 ) {
        B->import(
            qw(defstash curstash inc_gv dowarn
...
sub B::SV::ix {
    ...
    defined($ix) ? $ix : do {
        ...
        my $type  = $flags & 0xff;    # SVTYPEMASK
                                      ...
                                      #}
        asm "newsvx", $flags, $debug{Comment} ? sprintf( "type=%d,flags=0x%x,%s", $type, $flags, sv_flags($sv) ) : '';
        asm "stsv", $tix if $PERL56;
...
sub B::GV::ix {
    ...
    defined($ix) ? $ix : do {
        ...
        if (   ( $PERL510 and $gv->isGV_with_GP )
            or ( !$PERL510 and !$PERL56 and $gv->GP ) ) {    # only gv with gp
            ...
            if ( $PERL510
                ...
                $name = $gv->STASH->NAME . "::" . ( class($gv) eq 'B::SPECIAL' ? '_' : $gv->NAME );
            }
            nice "[GV $tix]";
            B::Assembler::maxsvix($tix) if $debug{A};
...
sub B::HV::ix {
    ...
    defined($ix) ? $ix : do {
        ...
        if ($name) {
            ...
            ( $i = not $i ) ? asm( "newpv", pvstring $_) : asm( "hv_store", $_ ) for @array;
            if ( VERSION < 5.009 ) {
                asm "xnv", $hv->NVX;
            }
...
sub B::PVIV::bsave {
    ...
    if ($PERL56) {
        ...
        asm "xiv", !ITHREADS
          && ( ( $sv->FLAGS & ( $SVf_FAKE | SVf_READONLY ) ) == ( $SVf_FAKE | SVf_READONLY ) ) ? "0 # but true" : $sv->IVX;
    }
}
...
sub B::PADLIST::bsave {
    ...
    nice "-PADLIST-", asm "ldsv", $varix = $ix unless $ix == $varix;
    asm "padl_name", $ix0;
    asm "padl_sym",  $ix1;
}
...
sub B::PMOP::bsave {
    ...
    if (ITHREADS) {
        ...
        $op->B::BINOP::bsave($ix);
        if ( !$PERL56 and $op->pmstashpv ) {    # avoid empty stash? if (table) pre-compiled else re-compile
            if ( !$PERL510 ) {
...
sub B::GV::bytecodecv {
    ...
    if ( $$cv && !( $gv->FLAGS & 0x80 ) ) {    # GVf_IMPORTED_CV / && !saved($cv)
        if ( $debug{cv} ) {
            warn sprintf(
                "saving extra CV &%s::%s (0x%x) from GV 0x%x\n",
...
";
    ...
    return sub {
        if ( $debug{-S} ) {
            ...
            for (
                qw(archname blversion ivsize ptrsize byteorder longsize archflag
                perlversion)
              ) {
