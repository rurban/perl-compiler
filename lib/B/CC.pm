...
# Disable with -fno-
my (
    $freetmps_each_bblock, $freetmps_each_loop, $inline_ops,    $opt_taint,      $opt_omit_taint,
...
sub cc_queue {
    ...
    my $fakeop = B::FAKEOP->new(
        "next" => $fakeop_next, sibling => 0, ppaddr => $name,
...
sub load_pad {
    ...
    for ( $ix = 1; $ix < @valuelist; $ix++ ) {
        ...
        $name = "${ix}_$name";
        $pad[$ix] = B::Stackobj::Padsv->new( $type, $flags, $ix, "i$name", "d$name" );
...
sub pp_and {
    ...
    if ( @stack >= 1 ) {
        ...
        if ( $bool =~ /POPs/ ) {
            ...
            runtime( sprintf( "if (!$bool) { PUSHs((SV*)%s); goto %s;}", $obj->as_sv, label($next) ) );
        }
    }
    else {
        save_or_restore_lexical_state($$next);
        runtime(
            sprintf( "if (!%s) goto %s;", top_bool(), label($next) ),
            "*sp--;"
        );
    }
    return $op->other;
...
sub pp_orassign {
    ...
    if ( @stack >= 1 ) {
        ...
        write_back_stack();
        save_or_restore_lexical_state($$next);
        runtime( sprintf( "PUSHs((SV*)%s); if ($bool) { goto %s; }", $obj->as_sv, label($next) ) );
    }
    else {
        save_or_restore_lexical_state($$next);
        runtime( sprintf( "if (%s) goto %s;", top_bool(), label($next) ) );
...
sub pp_dorassign {
    ...
    runtime(
        sprintf(
            "PUSHs(%s); if (%s && SvANY(%s)) goto %s;\t/* dorassign */",
    ...
    if ( $op->flags & OPf_MOD ) {
        ...
        if ( $private & OPpLVAL_INTRO ) {
            ...
            if ( $] >= 5.015002 ) {
                runtime(
                    sprintf(
                        "PL_curpad[%d] = Perl_vivify_ref(aTHX_ PL_curpad[%d], %d);",
                ...
                runtime(
                    sprintf(
                        "Perl_vivify_ref(aTHX_ PL_curpad[%d], %d);",
                        $ix, $private & OPpDEREF
                    )
                );
            }
...
sub bad_pp_anoncode {
    if ($inline_ops) {
        ...
        runtime '{ /* pp_anoncode */', '  CV *cv = MUTABLE_CV(PAD_SV(PL_op->op_targ));', '  if (CvCLONE(cv))', '    cv = MUTABLE_CV(sv_2mortal(MUTABLE_SV(Perl_cv_clone(aTHX_ cv))));', '  EXTEND(SP,1);', '  PUSHs(MUTABLE_SV(cv));', '}';
...
sub numeric_binop {
    ...
    $force_int ||=
      ( $flags & INTS_CLOSED && @stack >= 2 && valid_int( $stack[-2] ) && valid_int( $stack[-1] ) );
    if ( $op->flags & OPf_STACKED ) {
        ...
        if ( @stack >= 1 ) {
            ...
            if ($force_int) {
                ...
                runtime( sprintf( "sv_setiv(TOPs, %s);", &$operator( "TOPi", $$rightruntime ) ) );
            }
            else {
                my $rightruntime = B::Pseudoreg->new( "NV", "rnv" );
                runtime( sprintf( "$$rightruntime = %s;\t/* %s */", $right, $op->name ) );
                runtime( sprintf( "sv_setnv(TOPs, %s);", &$operator( "TOPn", $$rightruntime ) ) );
            }
        }
    }
    else {
...
sub numeric_unop {
    ...
    $force_int ||= ( $flags & INT_RESULT );
    $force_int ||=
      ( $flags & INTS_CLOSED && @stack >= 1 && valid_int( $stack[-1] ) );
    my $targ = $pad[ $op->targ ];
    $force_int ||= ( $targ->{type} == T_INT );
    if ($force_int) {
...
sub pp_enter {
    ...
    if ( 0 and $inline_ops ) {
        ...
        runtime(
            $] >= 5.011001 and $] < 5.011004 ? 'ENTER_with_name("block");' : 'ENTER;',
            "SAVETMPS;",
...
sub pp_require {
    ...
    my $ck_next = ${ $op->next } ? "PL_op != ($sym)->op_next && " : "";
    runtime(
        "while ($ck_next PL_op != (OP*)0 ) {",    #(test 28).
        "  PL_op = (*PL_op->op_ppaddr)(aTHX);",
        "  SPAGAIN;",
        "}"
    );
...
sub pp_grepstart {
    ...
    runtime(
        sprintf(
            "if (PL_op == (($sym)->op_next)->op_next) goto %s;",
            label($nexttonext)
        )
    );
...
sub import {
  ...
  OPTION:
    ...
    mark_skip(
        qw(B::C B::C::Flags B::CC B::Asmdata B::FAKEOP
