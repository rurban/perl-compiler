...
ctestok(
    ...
    ( $B::C::VERSION lt $when ? "TODO " : "" ) . "wrong ISA with empty packages fixed with B-C-$when"
);
ctestok(
    2, "C,-O3", 'ccode159i_r',
    '@X::ISA="Y";sub Y::z{"Y::z"} print "ok\n" if X->z eq "Y::z";delete $X::{z};exit',
    ( $B::C::VERSION lt $when ? "TODO " : "" ) . "wrong method dispatch by dumping unfound methods fixed with B-C-$when"
);
