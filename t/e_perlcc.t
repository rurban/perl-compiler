...
SKIP: {
  ...
  TODO: {
    ...
    is(
        `$perlcc --staticxs -S -o pcc -O3 -r -e "print q(ok)"  $devnull`, "ok",
...
isnt(
    `$perlcc --Wb=-fno-fold,-v -o pcc $f $redir`, '/Writing output/m',
...
TODO: {
    ...
    like(
        `$perlcc -B --Wb=-DG,-v -o pcc $f $redir`, "/-PV-/m",
        "-B -v5 --Wb=-DG -o file"
    );                                                                       #51
}
cleanup;
...
TODO: {
    local $TODO = "catch STDERR not STDOUT" if $^O =~ /bsd$/i;               # fails freebsd only
    like(
        `$perlcc -v5 $f $redir`, '/Writing output/m',
        "-v5 turns on -Wb=-v"
    );                                                                       #58
    like(
        `$perlcc -v5 -B $f $redir`, '/-PV-/m',
        "-B -v5 turns on -Wb=-v"
    );                                                                       #59
    like(
        `$perlcc -v6 $f $redir`, '/saving magic for AV/m',
        "-v6 turns on -Dfull"
    );                                                                       #60
    like(
        `$perlcc -v6 -B $f $redir`, '/nextstate/m',
...
TODO: {
    ...
    like(
        `$perlcc -o pcc --check -e"BEGIN{open(F,q(<xx))}"`,                                 #77
