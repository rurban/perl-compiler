...
Several years ago the undump functionality used to work on several 
...
Note that CORE favors C<.pmc> over C<.pm> files, so it would 
...
The attempt for a new perloptreeguts.pod manual:
