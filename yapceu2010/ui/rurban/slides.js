...
function createNotesWindow() { // creates a window for our notes
	if (!s5NotesWindow || s5NotesWindow.closed) { // Create the window if it doesn't exist
		...
		s5NotesWindow = window.open('ui/s5-notes.html', 's5NotesWindow', 'top=0,left=600');
	}
...
// Loads a note into the note window
  ...
  if (jl.selectedIndex < smax - 1) {
    nextTitle = jl.options[jl.selectedIndex + 1].text.replace(/^\d+\s+:\s+/, '') + ((jl.selectedIndex + 1) ? ' (' + (jl.selectedIndex + 1) + '/' + (smax - 1) + ')' : '');
  } else {
    nextTitle = '[end of slide show]';
  }
  ...
  if (s5NotesWindow && !s5NotesWindow.closed && s5NotesWindow.document) {
    ...
    s5NotesWindow.document.getElementById('next' + snum).innerHTML = nextTitle;
