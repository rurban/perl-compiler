...
[nbody](http://shootout.alioth.debian.org/u64q/performance.php?test=nbody)
performs a simple N-body simulation of the Jovian planets. 
Perl is currently by far the slowest, 26 min compared to 9-18 min for ruby,
php or python.
...
NV d1\_e is used. The IV part i1\_e is never used and not declared.
...
basically in local stack locations or even in registers, as faster
than doing ptr references to them. Initialising and writing back
seems to be okay and not exaggereted.
...
To compare real numbers, *500000* is the argument used at alioth, leading to 26 min.
My PC is faster:

    $ time perl5.14.2-nt ../shootout/bench/nbody/nbody.perl 500000
    -0.169075164
    -0.169096567
...
Compiled:
    ...
    perlcc --time -r -O -S -O1 --Wb=-fno-destruct,-Uwarnings,-UB,-UCarp \
           ../shootout/bench/nbody/nbody.perl 500000
...
So we bypassed python 3 and ruby 1.9, but not php and jruby.

Function calls are inspected in part 3, with the [binarytrees] benchmark.
It would also help the analyse loop here, as `sub analyse` can be easily
optimized.
