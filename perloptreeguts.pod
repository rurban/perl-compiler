...
are rehooked in C<Perl_linklist()> to the so-called B<exec> order. 
So the exec walk through the linked-list of ops is not too cache-friendly.

In detail C<Perl_linklist()> traverses the optree, and sets op-next pointers
to give the execution order for that optree. op-sibling pointers are
rarely unneeded after that.
...
See http://www.xray.mpe.mpg.de/mailing-lists/perl5-porters/2006-09/msg00082.html
...
op, a typical functional language usually holds this in the ops arguments.
So we are limited to max 32 lexical pragma hints or less. See L</Lexical Pragmas>.
...
or in F<dump.c>. C<Perl_runops_debug> is used with DEBUGGING and the faster
